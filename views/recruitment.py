...
recruitment.py
---------------
Click-to-filter: "Recruitment Source Performance" is clickable (when
recruitment_source data exists). Click a source and every other
chart/metric on this page narrows to it. Same session_state approach
as finance.py; see get_click_index() in filters.py.

recruitment_source / termination_reason / hire_date are each checked
independently with field_has_data() — checked against the PRE-cleaning
data on purpose: clean_standard_data() fills every missing categorical
value with "Unknown", so a field that's 100% absent would otherwise
look "present" as a single uninformative 'Unknown' bar. Turnover
itself (is_active) is always available regardless, since it's derived
during upload (see data_loader.add_is_active) — a real termination
date isn't required.
"""
...
HAS_SOURCE = field_has_data("recruitment_source")
HAS_REASON = field_has_data("termination_reason")
HAS_HIRE_DATE = field_has_data("hire_date")
...
if HAS_SOURCE:
    ...
    src_counts_sorted = src_counts.sort_values("count")  # the exact row order the chart is drawn in
    click_idx = get_click_index("recruit_src_chart")
    sel_source = (src_counts_sorted.iloc[click_idx]["recruitment_source"]
                  if click_idx is not None and click_idx < len(src_counts_sorted) else None)
else:
    sel_source = None
...
if HAS_HIRE_DATE and "termination_date" in df_view.columns and df_view["termination_date"].notna().any():
    st.divider()
...
if not (HAS_SOURCE or HAS_REASON or HAS_HIRE_DATE):
    st.info("This dataset doesn't include recruitment source, termination reason, or hire-date data — "
            "only the turnover metrics above are available.")
